...
export function BellIcon({ className, hasAlert = false }: IconProps & { hasAlert?: boolean }) {
  return (
    <svg viewBox="0 0 15.814 20" fill="currentColor" aria-hidden className={className ?? "h-5 w-[15.814px]"}>
      ...
      {hasAlert ? <circle cx="13.5" cy="2.8" r="2.6" fill="var(--mobile-color-state-holiday)" /> : null}
    </svg>
  );
}
